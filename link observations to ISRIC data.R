...
# load data
lonlat <- fread("dev/schutz2018_converted.csv")
...
dt <- dt[, XCEC := ((cec_mean_0_5 + cec_mean_5_15*2 + cec_mean_15_30*3)/6)]
dt <- dt[, XCLAY := ((clay_mean_0_5 + clay_mean_5_15*2 + clay_mean_15_30*3)/6)] 
dt <- dt[, xnit := ((ntot_mean_0_5 + ntot_mean_5_15*2 + ntot_mean_15_30*3)/6)]  
dt <- dt[, XPH := ((phw_mean_0_5 + phw_mean_5_15*2 + phw_mean_15_30*3)/6)]  
dt <- dt[, XSAND := ((sand_mean_0_5 + sand_mean_5_15*2 + sand_mean_15_30*3)/6)]   
dt <- dt[, XSILT := ((silt_mean_0_5 + silt_mean_5_15*2 + silt_mean_15_30*3)/6)] 
dt <- dt[, XSOC := ((soc_mean_0_5 + soc_mean_5_15*2 + soc_mean_15_30*3)/6)] 
dt <- dt[, xsom := (XSOC*2)]
...
dt[, pre_mn := pre_mn*12] # convert from mm/month to mm/y
...
final <- dt[,.(ID,lon,lat,XCEC,xnit,XPH,XSOC,XSAND,XSILT,XCLAY,pot_eva,mn_temp,prec_mn,xsom)]

# merge data with covariates into the data table with all rows of the converted data from Schutz et al. (2018)
final[, ID := NULL]
final <- final[, .SD[1], by = .(lon, lat)]
final <- merge(d1,final, by.x = c("X","Y"), by.y = c("lon","lat"), all.x=T)

# set order, round values and set names to lower
final <- final[order(ID)]
cols <- colnames(final[,!c("ID")])
final[, c(cols) := round(.SD,2), .SDcols = cols] 
setnames(final,tolower(colnames(final)))

# save the file  
#fwrite(final,'dev/covariates_schutz2018_formerge.csv', dec='.', sep=';')
